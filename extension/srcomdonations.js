...
var requestOptions = {
	follow: 1,
	headers: {
		'User-Agent': 'speedcontrol-srcomtracker'
	}
};
var eventID;
...
if (nodecg.bundleConfig && nodecg.bundleConfig.enable) {
	if (!nodecg.bundleConfig.slug) {
		nodecg.log.warn('Speedrun.com marathon integration is enabled, but the slug has not been set.');
		return;
	}
	...
	async.whilst(
		...
		function(callback) {
			needle.get(url, requestOptions, function(err, response) {
				...
				else if (err || !response || response.statusCode !== 200 || !response.body || !response.body.data)
